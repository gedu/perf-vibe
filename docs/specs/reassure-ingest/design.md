...
| `issues` field | Not persisted, not carried in the domain type | A third `issues_json` passthrough column | Proposal's Approach settled exactly two passthrough columns. Hook point: a `0007` adds `issues_json` (`0006` shipped as `kind`) if a diagnosis surface ever needs it. |

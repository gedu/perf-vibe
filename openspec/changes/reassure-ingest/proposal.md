...
|---|---|
| **No component field, no test-file field.** The only identity is the flat `name` (Jest's `describe > test` chain), so "by name" and "by test" are the same string. | Any component dimension is **derived**, never stored. No later phase may treat it as data. |

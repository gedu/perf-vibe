# Specification: `init` capability (Current — SHIPPED)

**Consolidated from**: SDD artifacts `docs/specs/init-command/{proposal,spec,design,tasks,verify-report}.md` (all complete), designed and merged across three chained PRs to `main`.

**Status**: COMPLETE. Capability ships with the following behavior as the authoritative current spec.
...
| `--driver`/`--db` verbatim pass-through (written only if supplied, no detection) | YES |
| `AndroidManifest.xml`/`build.gradle` detection | OUT OF SCOPE |
| Writing `sampler`/`marker_source` overrides, or auto-detecting `driver`/`db_path` | OUT OF SCOPE — code defaults already match a real Maestro setup; `--driver`/`--db` are trivial user-supplied pass-through only |
...
- `init` is pure CLI-adjacent scaffolding in `src/perf/cli/commands/init.py` with pure helpers (`parse_app_id`, `discover_flows`, `serialize_toml`, `merge_config`, `has_comments`) as module-level functions.
- appId parsing uses a minimal stdlib line-scan of the pre-`---` header (no YAML dependency).
- Flow discovery is recursive `rglob` for `*.yaml`/`*.yml`, excluding any path segment `== "subflows"` (case-insensitive).
- TOML write strategy: parse existing → deep-merge dict → full canonical re-serialize for collision detection and structural awareness.
- String escaping: prefer TOML literal strings `'…'` for `maestro_path`; fall back to basic `"…"` with escaping when the value contains `'` or a control character.
- `--json` contract: new `contracts/init_v1.py` with `schema_version=1`, independent of other contracts.
- `bundle_id` is always written when a concrete `appId` is detected; reserved-but-inert; mismatch-detection is a real correctness guard.
- Comment preservation: if the existing target `perf.toml` contains a `#` comment, interactive mode requires user confirmation; non-interactive/scripted mode requires `--force`, else exits `2`.
- Output path: reuses the existing global `--config` option as the write path; defaults to `./perf.toml` in CWD when `--config` is omitted.
- `--driver`/`--db`: verbatim literal pass-through, written only when supplied.
